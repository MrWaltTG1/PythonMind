...
class SecretCode():
    ...
    def create_pins(self):
        ...
        while i < self.settings.code_length:
            ...
            new_pin = Pin(self.settings,self.screen,pin_pos,random_color,self.settings.big_pin_radius)
            new_hidden_pin = Pin(self.settings,self.screen,pin_pos,self.settings.guess_pin_color_inactive,self.settings.big_pin_radius)
    ...
    def create_question_mark(self,pos):
        ...
        self.msg_image = self.font.render("?", True, self.settings.hud_colors["white"])
    ...
    def text_box(self):
        ...
        self.font = pygame.font.SysFont(self.settings.font_type, self.settings.font_size)
        self.msg_image = self.font.render(msg,True,self.settings.hud_colors["white"])
        self.msg_image_rect = self.msg_image.get_rect()
    ...
    def blitme(self):
        if self.hover and self.filled == 0:
            pygame.draw.rect(self.screen,self.settings.hud_colors['red'],self.bbox_rect,width=self.filled)
            ...
            for pin in self.pin_list:
                pin.blitme()
            pygame.draw.rect(self.screen,self.settings.hud_colors['red'],self.bbox_rect,width=self.filled)
            if self.filled == 0: self.screen.blit(self.msg_image, self.msg_image_rect)