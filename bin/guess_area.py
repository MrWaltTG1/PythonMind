...
class Guessbox():
    ...
    def create_pins(self):
        ...
        while i > 0:
            pos = (pos[0] + self.settings.big_pin_radius +
                   35, self.rect.midleft[1])  # type: ignore
    ...
    def create_buttons(self):
        ...
        self.confirm_box = Button(
            self.screen, self.settings, "Confirm", pos, "gb")
    ...
    def create_fake_guess(self):
        color_list = (self.settings.colors["red"], self.settings.colors["red"],
                      self.settings.colors["red"], self.settings.colors["red"])
        guess_dict = {
            "colors": color_list,
        }
        self.game_board.guess_list.append(guess_dict)
    ...
    def blitme(self):
        ...
        pygame.draw.rect(
            self.screen, self.settings.hud_colors['white'], self.rect, border_radius=2, width=4)
...
class Guesscolorpinarea():
    def __init__(self, screen, settings, pos):
        ...
        bbox_rect = (settings.guess_box_width / 3.5,
                     settings.guess_box_height - 10)
        ...
        for pin_pos in self.pin_pos_list:
            for index, colour in enumerate(self.settings.colors, start=1):
                if index == i:
                    ...
                    break
            i += 1
            new_pin = Pin(settings, screen, pin_pos,
                          pin_colour, self.radius, True)
