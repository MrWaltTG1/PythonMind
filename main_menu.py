...
class Main_menu():
    ...
    def create_buttons(self, button_text):
        ...
        for button_message in button_text:
            new_button = Button(self.screen, self.settings,
                                button_message, self.button_pos, "mm")
    ...
    def create_text(self):
        ...
        self.textbox_list1 = gf.create_text_box(
            self.settings, pos, size, text, font_size=100, text_color=self.settings.hud_colors["white"])
        ...
        self.textbox_list2 = gf.create_text_box(
            self.settings, pos, size, text, font_size=50, text_color=self.settings.hud_colors["white"])
        self.textbox_list_list.append(self.textbox_list2)

        pos = self.settings.rect.centerx, self.settings.rect.centery + 230
        text = "Can you crack the code?"
        self.textbox_list3 = gf.create_text_box(
            self.settings, pos, size, text, font_size=50, text_color=self.settings.hud_colors["white"])
...
class Options():
    ...
    def create_buttons(self, button_text: list) -> None:
        self.button_list = []
        self.button_pos = [(self.button_width / 2) + 20,
                           self.settings.screen_height - 35]
        for button_message in button_text:
            new_button = Button(self.screen, self.settings,
                                button_message, self.button_pos, "om")
            self.button_list.append(new_button)
            self.button_pos[1] -= self.button_height * 2
    ...
    def update_sliders(self):
        ...
        for slider in self.slider_list:
            ...
            percentage = 100 * \
                (slider.circle_pos[0] - slider.box_rect.left) / percent_max
...
class Start_menu():
    ...
    def create_buttons(self, button_text):
        ...
        for button_message in button_text:
            if button_message == "Back":
                self.button_pos = [(self.button_width / 2) +
                                   20, self.settings.screen_height - 35]
            elif button_message == "Start Game":
                self.button_pos = [(self.settings.screen_width - 20) -
                                   (self.button_width / 2), self.settings.screen_height - 35]
            elif button_message == "Hard" or button_message == "Normal":
                self.button_pos = [(self.settings.screen_width - 30) -
                                   (self.button_width / 2), self.diff_button_center]
            new_button = Button(self.screen, self.settings,
                                button_message, self.button_pos, "sm")
    ...
    def create_sliders(self):
        ...
        new_slider = Slider(self.settings, self.screen,
                            pos, mylist, is_time=True)
    ...
    def create_text_box(self):
        ...
        self.font = pygame.font.SysFont(
            self.settings.chalk_font_type, self.settings.chalk_font_size)
        self.text_rect_list = []
        for msg in msg_list:
            msg = msg.splitlines()
            self.msg_image = self.font.render(
                msg[0], True, self.settings.hud_colors['white'])
        ...
        if self.settings.difficulty == 2:
            for button in self.button_list:
                if button.msg == "Normal":
                    ...
                    button.rect.center = [
                        (self.settings.screen_width - 30) - (self.button_width / 2), self.diff_button_center]
